...
class FSDSoundScapesDataset(torch.utils.data.Dataset):  # type: ignore
    ...
    def __getitem__(self, idx):
        ...
        eventfile = os.path.join(sample_path, 'mixture.npy')
        labelfile = os.path.join(sample_path, 'label.npy')
        event_audio_list = np.load(eventfile)
        label_list = list(np.load(labelfile))

        gt_events = list(pd.read_csv(
            os.path.join(sample_path, 'gt_events.csv'), sep='\t')['label'])

        mixture = torch.sum(torch.from_numpy(event_audio_list), dim=0)
        ...
        for l in labels:
            idx = FSDSoundScapesDataset._labels.index(l)
            i_idx = label_list.index(idx)
            gt = gt + torch.from_numpy(event_audio_list[i_idx])

        maxval = torch.max(torch.abs(mixture))
        if maxval >= 1:
            mixture /= maxval
            gt /= maxval
