...
class ConvAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None, context_mask=None):
        ...
        if exists(mask) or exists(context_mask):
            ...
            dots.masked_fill_(mask == 1, mask_value)
...
class DilatedDenseNet(nn.Module):
    def __init__(self, depth=4, in_channels=64, freq_dim=129):
        ...
        for i in range(self.depth):
            ...
            setattr(
                self,
                "norm{}".format(i + 1),
                LayerNormFreqChannel(in_channels, freq_dim)
            )
...
class DenseEncoder(nn.Module):
    def __init__(self, in_channel, channels=64):
        ...
        self.conv_1 = nn.Sequential(
            ...
            LayerNormFreqChannel(channels, 257),
            nn.PReLU(channels),
        )
        self.dilated_dense = DilatedDenseNet(depth=4, in_channels=channels, freq_dim=257)
        self.conv_2 = nn.Sequential(
            nn.Conv2d(channels, channels, (1, 3), (1, 2), padding=(0, 1)),
            LayerNormFreqChannel(channels, 129),
            nn.PReLU(channels),
        )
...
class MaskDecoder(nn.Module):
    def __init__(self, num_channel=64, out_channel=1):
        ...
        self.dense_block = DilatedDenseNet(depth=4, in_channels=num_channel, freq_dim=129)
        self.sub_pixel = SPConvTranspose2d(num_channel, num_channel, (1, 3), 2)
        self.conv_1 = nn.Conv2d(num_channel, out_channel, (1, 2))
        self.norm = LayerNormFreqChannel(out_channel, 257)
        self.prelu = nn.PReLU(out_channel)
        self.final_conv = nn.Conv2d(out_channel, out_channel, (1, 1))
        # self.prelu_out = nn.PReLU(129, init=-0.25)
...
class LayerNormFreqChannel(nn.Module):
    def __init__(self, channel, freq_dim, eps=1e-6):
        super(LayerNormFreqChannel, self).__init__()
        self.layer_norm = nn.LayerNorm([channel, freq_dim])
...
class TSCB(nn.Module):
    ...
    def forward(self, x_in):
        ...
        attn_mask = self.get_mask(x_t.shape[1], x_t.device)
        x_t = self.time_conformer(x_t, attn_mask) + x_t
...
class TSCNet(nn.Module):
    def __init__(self, num_channel=32):
        ...
        # self.TSCB_5 = TSCB(num_channel=num_channel)

        self.enc, self.dec = make_enc_dec('stft',  # sample_rate 取 16000 和 8000 没有区别
                                          n_filters=512,
                                          kernel_size=512,
                                          stride=256,
                                          sample_rate=16000,
                                          window_type="hann")
        self.in_conv = DenseEncoder(2, 32)
        self.ou_conv = MaskDecoder(32, 2)
        self.label_proj = nn.Sequential(
            nn.Linear(256, 32 * 129),
            nn.LayerNorm(32 * 129),
        )
    ...
    def forward(self, x, label):
        ...
        out_1 = self.in_conv(batch)
        B, Q, T, C = out_1.shape
        label = self.label_proj(label)
        label = label.reshape([B, Q, C]).unsqueeze(2)
        out_2 = self.TSCB_1(out_1)
        out_3 = self.TSCB_2(out_2)
        out_4 = self.TSCB_3(out_3 + label)
        # out_5 = self.TSCB_4(out_4 + label)
        mask = self.ou_conv(out_4)

        batch = batch * mask
        B, Q, T, C = batch.size()
        batch = batch.view([B, 2, T, C]).transpose(2, 3)
...
class Net(nn.Module):
    def __init__(self, label_len, stft_chunk_size=160, stft_pad_size=120, embed_dim=256,
                 num_ch=2, D=64, B=6, I=1, J=1, L=0, H=128,
                 use_attn=False, lookahead=True, local_atten_len=100,
                 chunk_causal=False, num_src=2):
        super(Net, self).__init__()
        self.stft_chunk_size = 256
        self.stft_pad_size = 128
        self.num_ch = 1
        self.lookahead = lookahead

        # Input conv to convert x audio to a latent representation
        self.nfft = stft_chunk_size
...
if __name__ == "__main__":
    enc, dec = make_enc_dec('stft',  # sample_rate 取 16000 和 8000 没有区别
                            n_filters=256,
                            kernel_size=256,
                            stride=128,
                            sample_rate=16000,
    ...
    plt.plot(x.squeeze().detach().numpy())
    plt.plot(r.squeeze().detach().numpy())
    plt.show()
    print(torch.max(torch.abs(r - x)))
    # model = Net(label_len=8, stft_chunk_size=256, stft_pad_size=128)
    #
    # x = torch.randn((2, 1, 16000))
    # l = torch.randn((2, 256))
    # from thop import profile
    #
    # ops, pa = profile(model=model, inputs=(x, l))
    # print(ops / 1e9, pa / 1e6)
    # input_state = model.init_buffers(x.shape[0], x.device)
    # y, state = model(x, l, input_state)
    # print(y.shape)
