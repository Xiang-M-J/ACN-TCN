...
def load_checkpoint_with_new_scheduler(checkpoint, model, optim=None, lr_sched=None, data_parallel=False):
    ...
    if lr_sched is not None:
        ...
        sche_dict = {'step_size': 10, 'gamma': 0.1, 'base_lrs': [0.0005], 'last_epoch': state_dict['epoch']+1, 'verbose': False, '_step_count': state_dict['epoch'] + 2,
         '_get_lr_called_within_step': False, '_last_lr': [0.0005]}

        # state_dict['lr_sched_state_dict']['last_epoch'] = state_dict['epoch']
        lr_sched.load_state_dict(sche_dict)
